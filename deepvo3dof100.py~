...
def lstm(input, batch_size):
    ...
    output = tf.layers.dense(y_1_flattened, 3)
...
def main(self):
    ...
    image_batch, label_batch = tf.train.batch(
        [image, label], batch_size=128, capacity=2048)
    ...
    with tf.Session() as sess:
        ...
        sess.run(init)
        for bn in range(epoch_num):
            print ("Current batch number is {}".format(bn))
